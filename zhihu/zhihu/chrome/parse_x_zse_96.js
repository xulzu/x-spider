...
function ed(url_token, offset) {
  let ti = undefined;
  var tp = `101_3_3.0+/api/v4/members/${url_token}/followers?include=data%5B*%5D.answer_count%2Carticles_count%2Cgender%2Cfollower_count%2Cis_followed%2Cis_following%2Cbadge%5B%3F%28type%3Dbest_answerer%29%5D.topics&offset=${offset}&limit=20+AGDSHoNAnBmPTs1EWLYJ0KRD_THbLLxwYfY=|1732762623`;
...
function tv_build() {
  ("use strict");
  let window = new Proxy(window_, {
    get(t, k) {
      // console.log(k, typeof t[k], t[k])
      if (k === "__driver_unwrapped") {
        return void 0;
      }
      if (k === "__fxdriver_evaluate") {
        return void 0;
      }
      if (k === "__selenium_evaluate") {
        return void 0;
      }
      if (k === "__webdriver_script_function") {
        return void 0;
      }
      if (k === "__webdriver_evaluate") {
        return void 0;
      }
      if (k === "_selenium") {
        return void 0;
      }
      if (k === "_Selenium_IDE_Recorder") {
        return void 0;
      }
      if (k === "spawn") {
        return void 0;
      ...
      if (k === "_sessionHistory") {
        return void 0;
      }
      if (k === "_resourceLoader") {
        return void 0;
      }
      if (k === "patchRequire") {
        return void 0;
...
function ty() {
  var tu = function (tt, te) {
    ...
    tg = function (tt) {
      var te,
        tr = [];
      for (te = 0, tr[(tt.length >> 2) - 1] = void 0; te < tr.length; te += 1)
        tr[te] = 0;
      var ti = 8 * tt.length;
      for (te = 0; te < ti; te += 8)
    ...
    tb = function (tt, te) {
      ...
      return (
        (ti = tA(tu.concat(tg(te)), 512 + 8 * te.length)),
    ...
    tw = function (tt) {
      ...
      for (tr = 0; tr < tt.length; tr += 1)
        ta +=
          ti.charAt(((te = tt.charCodeAt(tr)) >>> 4) & 15) + ti.charAt(15 & te);
      return ta;
    },
    t_ = function (tt) {
...
function parse_x_zse_96([url_token, total]) {
  let res = [];
  for (let i = 0; i < total; i += 20) {
    res.push(ed(url_token, i));
  }
  return res;
}
return parse_x_zse_96(arguments);
